...
public class LocationHelper {
    ...
    public static float[] WSG84toECEF(GpsEntity entity) {
        double radLat = Math.toRadians(entity.getWgLat());
        double radLon = Math.toRadians(entity.getWgLon());
        ...
        float x = (float) ((N + entity.getAltitude()) * clat * clon);
        float y = (float) ((N + entity.getAltitude()) * clat * slon);
        float z = (float) ((N * (1.0 - WGS84_E2) + entity.getAltitude()) * slat);

        return new float[] {x , y, z};
    }

    public static float[] ECEFtoENU(GpsEntity currentLocation, float[] ecefCurrentLocation, float[] ecefPOI) {
        double radLat = Math.toRadians(currentLocation.getWgLat());
        double radLon = Math.toRadians(currentLocation.getWgLon());
